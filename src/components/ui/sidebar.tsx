...
>(
  (
    ...
    // Set initial state from cookie on client mount
    React.useEffect(() => {
      if(isClient) {
...
>(({ side = "left", collapsible = "icon", className, children, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn(
        "hidden md:flex md:flex-col transition-all duration-200 ease-in-out bg-card border-r",
  ...
  (
    ...
    const button = (
      <Comp
        ...
        className={cn("h-auto p-3", className)}
        {...props}
      />
    )
